...
import { hotelRegister, getCurrentUser } from "@/api/user";
import { ref } from "vue";
interface hotel {
  id: string;
  idCard: string;
...
const roomImage = new URL("@/img/亲子房.jpg", import.meta.url).href;
const roomType = "温馨亲子房";
const roomCount = 50;
...
const handleSubmit = async () => {
  ...
  if (res.data.data) {
    ...
    message.error("预定失败" + res.data.description);
  }
};
...
const ReserveRoom = async () => {
  ...
  if (res.data.data) {
    ...
    Hotel.phoneNumber = user.userAccount;
  } else {
    console.log(res.data.data);
    message.error("请先登录" + res.data.description);
  }
};
</script>
